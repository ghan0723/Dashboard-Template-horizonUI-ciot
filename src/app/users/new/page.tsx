...
import { compareRange, validateIPRange } from 'utils/valid';
...
export default function SignIn() {
    ...
    const fetchPrivilegeAndRange = async () => {
        ...
        try {
            ...
            setPrivilege((result[0].privilege === 1 && result[0].id === 1) ? '1' : (result[0].privilege === 1 ? '2' : '3'));
    ...
    const handleSubmit = async (event: any) => {
        ...
        if (username.length < 5 || username.length > 15) {
            ...
            userSwal(5, 'new');
            event.preventDefault();
        } else if (!validateIPRange(range,cookieRange)) {
